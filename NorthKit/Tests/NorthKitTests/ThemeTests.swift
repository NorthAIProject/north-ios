...
struct ThemeTests {
    @Test func everyTokenResolvesFromTheCatalog() {
        ...
        for name in names {
            #expect(UIColor(named: name, in: .module, compatibleWith: nil) != nil, "missing colour set \(name)")
        }
    }

    @Test func lightAndDarkBackgroundsDiffer() throws {
        let colour = try #require(UIColor(named: "Background", in: .module, compatibleWith: nil))
