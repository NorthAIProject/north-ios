...
# signs in and retries only the two model-backed steps (the plan and the coach
# conversation), so the account is not filled with duplicates. Each step prints
# ok or the HTTP status, so a partial seed is visible.
set -euo pipefail
...
try:
    ...
    existed = True
    print("ok   login (account existed): retrying the plan and the conversation only")
